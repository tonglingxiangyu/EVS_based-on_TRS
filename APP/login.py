...
class LoginWindow(QWidget):
    ...
    def initUI(self): 
        ...
        logomap = QPixmap(dirpath+'/../image/signin.jpeg')
        ...
        self.setWindowIcon(QIcon(dirpath+'/../image/login.jpeg'))
    ...
    def onLogin(self):
        ...
        if usr == '':
            ...
            flag,role = login(usr, pwd)
            if flag == 1:
                write_log_to_Text('用户'+usr+'登录成功')
                if self.mainWindow is not None:
                    self.close()
                    self.mainWindow.usr = usr
                    self.mainWindow.role = role
