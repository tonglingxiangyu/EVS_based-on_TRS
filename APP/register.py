...
class RegisterWindow(QWidget):
    ...
    def initUI(self):
        ...
        formlayout.addRow(role, self.roleInput)
        ...
        self.setWindowIcon(QIcon(dirpath+'/../image/record.png'))
    ...
    def onRegister(self):
        ...
        if usr == '':
            ...
            QMessageBox.warning(self, 'warning', '请输入您的角色', QMessageBox.Yes)
        elif cpwd != pwd:
            QMessageBox.warning(self, 'warning', '两次输入的密码不一致', QMessageBox.Yes)
        else:
            flag = register(usr, pwd, role)
            if flag == 1:
                write_log_to_Text('用户' + usr + '注册成功')
