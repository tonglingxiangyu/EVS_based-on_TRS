...
class VoteWindow(QWidget):
    ...
    def initUI(self):
        ...
        self.confirmButton.setIcon(QIcon(dirpath+'/../image/enter.png'))
        ...
        self.setWindowIcon(QIcon(dirpath+'/../image/vote.jpg'))

    def onConfirm(self):

        captcha = self.captchaInput.text()
        if captcha == '':
            QMessageBox.warning(self, 'warning', '请输入邀请码', QMessageBox.Yes) # captcha=md5(title+usr+votelimit+choice+tag)
        else:
