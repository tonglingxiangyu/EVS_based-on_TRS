...
class ViewWindow(QWidget):
    ...
    def initUI(self):
        ...
        self.showButton.setIcon(QIcon(dirpath+'/../image/search.png'))
        self.showButton.clicked.connect(self.onShow)
        
        self.delButton = QPushButton('删除活动')
        self.delButton.setFont(QFont('微软雅黑'))
        self.delButton.setIcon(QIcon(dirpath+'/../image/delete.jpg'))
        ...
        self.setWindowIcon(QIcon(dirpath+'/../image/view.png'))
    ...
    def onShow(self):
        for i in range(self.rownum):
            if self.radioButtons[i].isChecked() == True:
                ...
                res = showVoteResult(captcha, total)
                drawResult(res)
