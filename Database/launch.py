...
def insert_launch(usr, title, captcha, votelimit, total):
    ...
    sql = """insert into launch(usr, id, title, captcha, votelimit, total)\
     values('{}', {}, '{}', '{}', {}, "{}")""".format(usr, id, title, captcha, votelimit, total)
    flag, e = op_mysql(sql)
    if flag == False:
        print(e)
        return 0 #数据库端错误
...
def updatevote(captcha, choice):
    '''
    最新的用来实时更改投票结果
    '''
    sql = "select total from launch where captcha='{}'".format(captcha)
    flag, res = op_mysql(sql)
    print(res)
    for i in res:
        b=i[0]
    ...
    sql = """update launch set total = "{}" where captcha = '{}'""".format(total, captcha)
    flag, res = op_mysql(sql)
    return flag
